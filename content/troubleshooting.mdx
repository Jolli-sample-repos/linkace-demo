...
{/* Build timestamp: 1765296473813 - Forces cache invalidation */}
...
If you have issues with LinkAce or run into any errors, please follow these steps before creating a discussion or an issue on Github. You can find some common issues below the troubleshooting guide.

## Disclaimer

As written in the Readme of the project and on the website: I built LinkAce to solve my problem, and I now share my solution and code to you without charging anything. I spent a lot of my free time building this application already, so I won't offer any free personal support, customization or installation help. If you need help please visit the community forum and post your issue there.

## Help others to help you

Generally speaking: if you have a problem, help others to understand your problem.

It is not helpful if you just post an error message like "There was an error". Post details about your setup, what you did to make it work already, and what you expected. After following the troubleshooting guide, you will be able to post much more information about your LinkAce instance, which in turn might help others understand your problem and help you fix it.

The golden rule above everything else is: be nice and patient.
...
If LinkAce is running, you can find the general system logs under System Logs in the menu.If LinkAce is not running or not accessible, you find the same logs under ./storage/logs.

### Getting logs from within Docker

If you run LinkAce via Docker, you may use the following commands to access your container.

> **Note:** Docker setups may vary. The container names can be different depending on where and how you set up LinkAce. You can find the name of your container by running `docker ps`.

```bash
# For single Docker containers
docker exec -it [your LinkAce container name] sh
...
If you aren't able to solve the issue with the help of the debug mode or logs, you might search the community forum for your error message. There are tons of threads about all kinds of issues already and yours might already be solved.

## 4. Did you try turning it off and on again?

As bland as it sounds, please try to restart LinkAce. If that didn't help, try to restart your server.
...
Important: If you want to share tons of log files and additional info, please for the sake of god use Pastebin to drop off all the files and logs there. It is not helpful for others to encounter threads with 1000+ lines of logs all in the thread itself.

### Questions to consider when posting:

What does your tech stack look like?If you run LinkAce behind a proxy or load balancer, please share details about it, and logs from it if necessary.Do you run Cloudflare or another provider in front of LinkAce?Are you using an SSO provider? If yes, share details about the setup and post logs if necessary.

### Gather additional system information

The instructions are written for Linux systems. If you use something else, please look up how to find details about it.
...
There are a few common issues that might occur when you run LinkAce based on its architecture and the technology it uses.

### Permission issues

The most common issue is permissions. There are two major parts of LinkAce which require proper permissions:
...
Note: Please note that there will be no help for changing the password inside your database. Please refer to the official documentation of your database server.
...
## Resetting the database (if not yet set up)
...
Default password: The default password shipping with LinkAce is ChangeThisToASecurePassword!